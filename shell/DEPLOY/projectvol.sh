...
RESULTFILE="/Users/andi/$CLAY/folderlist.txt"
...
then
    ...
    df | awk '/CLAYNET/ { print $9 " -> " $1}' | sed  "s/CLAY_EXT\/$USERSDIR\/andi\/$CLAY\/PROJECTVOL_SRC/PROJECTVOL/" | sed  "s/Users\/andi\/$CLAY\/PROJECTVOL_SRC/Volumes\/PROJECTVOL/" | tee -a $RESULTFILE
    if ! [ $LAN_PROJECT -eq 1 ]; then
