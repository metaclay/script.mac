...
TEMPDIR="/Users/andi/$CLAY/__TEMPDIR__"
...
unmount() {
    ...
    echo
    if ! [ -d "/Users/$USER/$CLAY" ]; then
        printf "> mkdir /Users/$USER/$CLAY "
        mkdir /Users/$USER/$CLAY && echo "... OK"  || exit 1
    else
        echo "/Users/$USER/$CLAY ... OK"    
    fi
...
LOCALIZED_CLAYNET_SRC="$CLAY_HOME/CLAYNET_SRC"
...
then
    ...
    printf "> mount -> //$IPADDR/CLAYNET/homes/andi/projects -> $PROJECTVOL_SRC/projects"
    mount_smbfs //$USER:$PASSWORD@$IPADDR/CLAYNET/homes/andi/projects $PROJECTVOL_SRC/projects  && echo "... OK" || exit 1
...
ls -ls $PROJECTVOL | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
if [ $LAN -gt 0 ];
then
    if ! [ $LAN_CLAYNET -eq 1 ]; then
        ls -ls -d $PROJECTVOL/* | grep -v "projects\|localized\|elements\|render\|temp" | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
    fi
    df | awk '/CLAYNET/ { print $9 " -> " $1}' | sed  "s/CLAY_EXT\/__USERS__\/andi\/$CLAY\/PROJECTVOL_SRC/PROJECTVOL/" | sed  "s/Users\/andi\/$CLAY\/PROJECTVOL_SRC/Volumes\/PROJECTVOL/" 
    if ! [ $LAN_PROJECT -eq 1 ]; then
        ls -ls -d $PROJECTVOL/* | grep -v "CLAYNET\|localized\|elements\|render\|temp" | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
    fi
    ls -ls -d $PROJECTVOL/localized/_Volumes/PROJECTVOL/CLAYNET | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
    PARAM="CLAYNET\|projects\|localized"


    ls -ls -d $PROJECTVOL/* | grep -v $PARAM | awk '{print $10 $11 $12}'

else
    ls -ls -d $PROJECTVOL/* | grep 'CLAYNET\|projects' | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
    ls -ls -d $PROJECTVOL/localized/_Volumes/PROJECTVOL/CLAYNET | awk '{print $10 $11 $12}' | sed -e 's/->/ -> /g' 
    ls -ls -d $PROJECTVOL/* | grep -v 'CLAYNET\|projects' | grep -v 'localized' | awk '{print $10 $11 $12}'
